...
export default function CreateTournament() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      if (points > 0) {
        const percent = prizePoolNum > 0 ? ((points / prizePoolNum) * 100).toFixed(1) + '%' : '0%';
        distribution.push({
          rank: `Top ${i}`,
          points,
          percent,
    ...
    try {
      ...
      if (showPrizeDistribution && distribution.length > 0) {
        createData.distribution = distribution;
      }

      await addDoc(collection(db, 'tournaments'), createData);
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="bg-[#131B2F] border border-gray-800 rounded-[2.5rem] p-6 lg:p-10 space-y-8 shadow-2xl relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="md:col-span-2 flex items-center justify-between bg-[#0B1121] p-3 rounded-lg border border-gray-800">
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                <input 
                  ...
                  onChange={(e) => setShowPrizeDistribution(e.target.checked)}
            ...
            {showPrizeDistribution && (
              <PrizeDistributionInputs prizePool={Number(prizePool)} />
            )}
