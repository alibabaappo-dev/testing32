...
// Auth Routes (Forgot Password, Verify Code, Reset Password)
...
router.post('/auth/verify-code', async (req, res) => {
  try {
    ...
    if (!resetSnap.exists || resetSnap.data()?.code !== code) return res.status(400).json({ error: 'Invalid code' });
...
router.post('/send-push', async (req, res) => {
  try {
    ...
    const message = { notification: { title, body }, tokens: [...new Set(tokens)].slice(0, 500) };
...
// On Netlify, handle root path for serverless-http
if (process.env.NETLIFY) {
  app.use('/', router);
}

// Local Server & Vite Setup (Only for Local/Dev)
if (!process.env.NETLIFY) {
  const init = async () => {
